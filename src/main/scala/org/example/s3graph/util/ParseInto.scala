    ...
    parse(jsonString) match {
      case Right(parsed) =>
        parsed.as[T].left.map { failure =>
          logger
            .error(s"Circe decoding failure: ${failure.getMessage()}. Original String: $jsonString")
          ParsingException(failure.getMessage())
        }
      case Left(error: ParsingFailure) =>
        logger.error(
          s"Parsing failure. Error message: ${error.getMessage()}. Original String: $jsonString"
        )
