...
object S3Client {
  ...
  def multipartUpload(inputStream: ByteArrayInputStream, bucket: String, key: String)(
    implicit materializer: Materializer, ec: ExecutionContext
  ): Future[Option[MultipartUploadResult]] =
    StreamConverters
      .fromInputStream(() => inputStream, chunkSize = 100000)
      .toMat(S3.multipartUpload(bucket, key))(Keep.right).mapMaterializedValue(_.map(x => Some(x))).run()
